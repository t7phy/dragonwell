...
class BCSummaryTool
{
 public:
   ...
   BCSummaryPriorModel * GetPriorModel()
	 ...
	 * Print a summary plot for the parameters.
	 * @return An error flag. */
	int PrintNuisancePlot(const char * filename = "parameters.eps", int firstNuisanceParIndex=5 );   
	//int PrintNuisancePlot(const char * filename = "nuisance.eps", int firstNuisnaceParIndex = 0);
