#ifndef ThreadRunPythiaWW_cxx
#define ThreadRunPythiaWW_cxx
...
void ThreadRunPythiaWW(Control process)
{
     ...
     loopPythiaWW *Run[TotalThread];
     TreeForPythiaWW *PythiaWWTree[TotalThread];
     HistsPythiaWW* myhists;

     myhists = new HistsPythiaWW();
     myhists->openFile(process.SaveName);
     myhists->LinkLog(process.myLog);
     myhists->bookHists(TotalThread);

     for(int j = 0; j < TotalThread; j++){
       PythiaWWTree[j] = new TreeForPythiaWW();
       PythiaWWTree[j]->InputFile(process.RootNames);

       Run[j] = new loopPythiaWW(process.RootType);
     ...
     for(int iround = 0; iround < process.TotalRound; iround++){
       ...
       for(int j = 0; j < TotalThread; j++){
         if(!OneFilePerJob) PythiaWWTree[j]->ConfigFile(process.NFilePerJob, iround);

         if(OneFilePerJob){
           ifile = iround * TotalThread + j;
           if((iround == process.TotalRound - 1) && ifile >= process.TotalFileNumber) continue;
           PythiaWWTree[j]->ConfigFile(ifile);
         }

         Run[j]->InputTree(PythiaWWTree[j]);
...
void CondorRunPythiaWW(Control process)
{
     ...
     PythiaWWTree = new TreeForPythiaWW();
     PythiaWWTree->InputFile(process.RootNames);
     PythiaWWTree->ConfigFile(process.NFilePerJob, 0);

     Run = new loopPythiaWW(process.RootType);
     Run->InputHist(myhists);
     Run->InputTree(PythiaWWTree);
