...
#include "BATHelper/BCComparisonTool.h"
...
std::vector<TH1D> BCMTFAnalysisFacility::BuildEnsemble(std::vector<double> parameters)
{
	...
	for (int ichannel = 0; ichannel < fMTF->GetNChannels(); ++ichannel) {
		...
		std::string bgProcName = channel->GetName()+"Bkg";
		TH1* bgHist = channel->GetTemplate(fMTF->GetProcessIndex(bgProcName.c_str()))->GetOriginalHistogram();
		...
		for (int ibin = 1; ibin <= nbins; ++ibin) {
			...
			if (fMTF->GetFlagDoStatErr() && ibin <= channel->GetMaxBinStatErr())
			  do
			    observation = fRandom->Gaus(expectation,TMath::Sqrt(TMath::Power(bgHist->GetBinError(ibin),2) + expectation));
			  while (observation < 0.0);
			else
			  observation = fRandom->PoissonD(expectation);//yfu
                          //observation = expectation;//yfu no random for test

			hist.SetBinContent(ibin, observation);
//cout<<"BuildEnsemble: ichannel: "<<ichannel<<" ibin: "<<ibin<<" expectation: "<<expectation<<" observation: "<<observation<<endl;
		}
...
TTree* BCMTFAnalysisFacility::BuildEnsembles(TTree* tree, int nensembles)
{
	...
	for (int i = 0; i < nparameters; ++i) {
		//tree->SetBranchAddress(Form("Parameter%i", i), &(parameters[i]));
                tree->SetBranchAddress(fMTF->GetParameter(i).GetName().c_str(), &(parameters[i]));//yfu
                tree->SetBranchAddress("Phase", &phase);//yfu
	}
	...
	for (int iensemble = 0; iensemble < nensembles; ++iensemble) {
		// get random event from tree
                int index;
                while(phase == -1){
		  index = (int) fRandom->Uniform(tree->GetEntries());
		  tree->GetEntry(index);
                }
                phase = -1;

		// create ensembles
                //parameters.at(2) = 5.0;//yfu for test
...
// ---------------------------------------------------------
TTree* BCMTFAnalysisFacility::PerformEnsembleTest(TTree* tree, int nensembles, const std::vector<int>& details)
{
	...
	//BCLog::SetLogLevel(BCLog::nothing);
        ...
        fMTF->WriteMarkovChain(false);
        fMTF->SetPrecision(BCEngineMCMC::kMedium);//yfu
        fMTF->SetNChains(5);//yfu
	...
	std::vector<double> out_std_marginalized(nparameters);
        std::vector<double> out_L_at_mode_marginalized(nparameters);
        std::vector<double> out_L_at_minpar_marginalized(nparameters);
        std::vector<double> out_L_at_maxpar_marginalized(nparameters);
	...
	for (int i = 0; i < nparameters; ++i) {
		...
		if (fFlagMCMC) {
			...
			tree_out->Branch(Form("std_marginalized_%i", i), &out_std_marginalized[i], Form("marginalized std of par. %i/D", i));
                        if(details.size()>(unsigned int)i && details[i]>=1){
                          tree_out->Branch(Form("L_at_mode_marginalized_%i", i),&out_L_at_mode_marginalized[i], Form("marginalized likelihood value at mode par. %i/D", i));
                          tree_out->Branch(Form("L_at_minpar_marginalized_%i", i),&out_L_at_minpar_marginalized[i], Form("marginalized likelihood value at min range par. %i/D", i));
                          tree_out->Branch(Form("L_at_maxpar_marginalized_%i", i),&out_L_at_maxpar_marginalized[i], Form("marginalized likelihood value at max range par. %i/D", i));
                          if(details[i]>=2){
                            tree_out->Branch(Form("marginalized_posterior_L_%i",i),fMTF->GetMarginalized( fMTF->GetParameter(i).GetName() ).GetHistogram());
                          }
                        }
		}
	}
        for(int ibin = 1; ibin <= nbins_matrix[0].size(); ibin++){
           tree_out->Branch((TString)"Pseudodata_bin_" + (int)ibin, &nbins_matrix[0][ibin - 1], (TString)"Pseudodata_bin_" + (int)ibin + (TString)"/D");
        }
        FittedBkg.resize(nbins_matrix[0].size());
        for(int ibin = 1; ibin <= nbins_matrix[0].size(); ibin++){
           tree_out->Branch((TString)"Background_bin_" + (int)ibin, &FittedBkg[ibin - 1], (TString)"Background_bin_" + (int)ibin + (TString)"/D");
        }
	...
	for (int iensemble = 0; iensemble < nensembles; ++iensemble) {
		...
		tree->GetEntry(index);
                ...
                for(int ip = 0; ip < out_parameters.size(); ip++){
                  cout<<"input parameter "<<ip + 1<<": "<<out_parameters.at(ip)<<endl;
                }
		...
		if (fFlagMCMC) {
			...
			fMTF->FindMode( fMTF->GetBestFitParameters() ); 
                        ...
                        //yfu
                        //fMTF->PrintAllMarginalized((std::string)((fMTF->GetSafeName() + (TString)"_Ensemble_" + (int)iensemble + "_plots.pdf").Data()));
                        //fMTF->PrintKnowledgeUpdatePlots((std::string)((fMTF->GetSafeName() + (TString)"_Ensemble_" + (int)iensemble + "_update.pdf").Data()));
		}
		else {
			// find mode
			fMTF->FindMode();
		...
		for (int i = 0; i < nparameters; ++i) {
			if (fFlagMCMC) {
				...
				//out_std_marginalized[i]=hist->GetRMS();
                                BCH1D *hist_ptr = nullptr;
                                BCH1D hist = (fMTF->GetMarginalized( fMTF->GetParameter(i).GetName() ));//yfu
                                hist_ptr = &hist;
                                TH1D* TH1D_ptr = (TH1D *)hist.GetHistogram();
                                if(TH1D_ptr){
                                  double mode = hist.GetHistogram()->GetBinCenter(hist.GetHistogram()->GetMaximumBin());
                                  if(details.size()>(unsigned int)i && details[i]>=1){
                                    out_L_at_mode_marginalized[i]=TH1D_ptr->GetBinContent(TH1D_ptr->FindBin(mode));
                                    out_L_at_minpar_marginalized[i]=TH1D_ptr->GetBinContent(1);
                                    out_L_at_maxpar_marginalized[i]=TH1D_ptr->GetBinContent(TH1D_ptr->GetNbinsX());
                                  }
                                  out_mode_marginalized[i] = hist.GetHistogram()->GetBinCenter(hist.GetHistogram()->GetMaximumBin());
                ...
                for(int ibin = 1; ibin <= nbins_matrix[0].size(); ibin++){
                  FittedBkg[ibin - 1] = fMTF->Expectation(0, ibin, out_mode_global);
                }
		...
		for (int ichannel = 0; ichannel < nchannels; ++ichannel) {
			...
			out_cash_mode_total += out_cash_mode[ichannel];
                ...
                for(int ip = 0; ip < out_mode_global.size(); ip++){
                  cout<<"output parameter global "<<ip + 1<<": "<<out_mode_global.at(ip)<<endl;
                }
                for(int ip = 0; ip < out_mode_marginalized.size(); ip++){
                  cout<<"output parameter marginalize "<<ip + 1<<": "<<out_mode_marginalized.at(ip)<<endl;
                }
                for(int ip = 0; ip < out_95quantile_marginalized.size(); ip++){
                  cout<<"95% quantile marginalized: "<<ip + 1<<": "<<out_95quantile_marginalized.at(ip)<<endl;
                }
...
int BCMTFAnalysisFacility::PerformSingleChannelAnalyses(const char* dirname, const char* options)
{
	...
	for (int i = 0; i < nparameters; ++ i) {
		...
		//BCComparisonTool* ct_mcmc = new BCComparisonTool(fMTF->GetParameter(i)->GetName().c_str());
                BCComparisonTool* ct = new BCComparisonTool(fMTF->GetParameter(i).GetName().c_str());//yfu
                BCComparisonTool* ct_mcmc = new BCComparisonTool(fMTF->GetParameter(i).GetName().c_str());
	...
	for (int i = 0; i < nparameters; ++ i) {
		...
		if (flag_mcmc) {
			...
			//												 hist->GetMean(),
			//												 hist->GetRMS());
                        BCH1D hist = fMTF->GetMarginalized( i );//yfu
