/*
 * Copyright (C) 2008-2012, Daniel Kollar and Kevin Kroeninger.
 * All rights reserved.
 *
 * For the licensing terms see doc/COPYING.
 */

// ---------------------------------------------------------

#include <TH1D.h>

#include "BAT/BCLog.h"
#include "BAT/BCH1D.h"

#include "BATHelper/BCSummaryPriorModel.h"

// ---------------------------------------------------------
BCSummaryPriorModel::BCSummaryPriorModel()
   : BCModel()
   , fTestModel(0)
{
 //yfu
 SetMarginalizationMethod(BCIntegrate::kMargMetropolis);
 SetOptimizationMethod(BCIntegrate::kOptMinuit);
 SetRandomSeed(12345678);
}

// ---------------------------------------------------------
BCSummaryPriorModel::BCSummaryPriorModel(const char * name)
   : BCModel(name)
   , fTestModel(0)
{
}

// ---------------------------------------------------------
BCSummaryPriorModel::~BCSummaryPriorModel()
{}

// ---------------------------------------------------------
void BCSummaryPriorModel::SetModel(BCModel * model)
{
   fTestModel = model;

   // copy parameters
   int npar = fTestModel->GetNParameters();
   for (int i = 0; i < npar; ++i) {
      BCParameter par = fTestModel->GetParameter(i);
      AddParameter(par);
   }

   // set default histogram binning to the one of the original model
   for (int i = 0; i < npar; ++i) {
      // this construct has to go here, because otherwise there is a
      // warning from BCEngineMCMC:: MCMCGetH1Marginalized
      if (fTestModel->GetBestFitParameters().size() > 0){
         BCH1D* histptr = nullptr;
         BCH1D hist = fTestModel->GetMarginalized( i );
         histptr = &hist;
         if (histptr) {
            int nbins = hist.GetHistogram()->GetNbinsX();
            //SetNbins( (fTestModel->GetParameter(i).GetName()).c_str(), nbins);
            SetNbins(nbins);
            //GetVariable("signal").SetNbins(nbins);//yfu
         }
      }
   }

   // set default MCMC setup to the one of the original model
   SetNChains( fTestModel->GetNChains() );
   SetNLag( fTestModel->GetNLag() );
   SetNIterationsMax( fTestModel->GetNIterationsMax() );
   SetNIterationsRun( fTestModel->GetNIterationsRun() );
   SetNIterationsPreRunMin( fTestModel->GetNIterationsPreRunMin() );
   //SetNIterationsUpdate( fTestModel->GetNIterationsUpdate() );//yfu
   //SetNIterationsUpdateMax( fTestModel->GetNIterationsUpdateMax() );//yfu
   //SetRValueCriterion( fTestModel->GetRValueCriterion() );//yfu
   SetRValueParametersCriterion( fTestModel->GetRValueParametersCriterion() );

}

// ---------------------------------------------------------
double BCSummaryPriorModel::LogLikelihood(const std::vector<double> & parameters)
{
   return fTestModel->LogAPrioriProbability(parameters);
//   return fTestModel->LogLikelihood(parameters);
}

// ---------------------------------------------------------
double BCSummaryPriorModel::LogAPrioriProbability(const std::vector<double> & /*parameters*/)
{
   return 0;
}

// ---------------------------------------------------------

