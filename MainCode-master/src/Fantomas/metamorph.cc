...
double ControlPoint::f() 
{
  ...
  switch (MappingMode) 
  {
    ...
    case 2: //bounded softsign scaling
      ...
      break;
    */
    case -1: // yfu: match to CT18 ubar, dbar, str 
      ftmp = sf;
      break;
    default:
...
double metamorph::yx(const double x){
  ...
  if(MappingMode == -1){// yfu: match to CT18 ubar, dbar, str
    double base = 1.0 / (1.0 / (pow(sqrt(x), stretchPower)+
                                pow(xstrmin, stretchPower))
                         + (1.0 - pow(xstrmax, stretchPower))
                         / pow(xstrmax, stretchPower));

    xstretched = pow(base, 1.0/stretchPower);

    xstretched = 1.0 - xstretched;
  }

  double result = pow(xstretched, xPower);
...
int metamorph::SetBoundary(const int MappingModeIn, double fmIn[], double fpIn[]) 
{
  ...
  MappingMode = MappingModeIn; // yfu add for saving MappingMode
