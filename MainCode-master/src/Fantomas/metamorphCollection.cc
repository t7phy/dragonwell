#include "Fantomas/metamorphCollection.h"
...
void metamorphCollection::ReadCard(const string& inputcard)
{
  ...
  while (true){
    ...
    if (MappingMode[NMeta] !=0 && MappingMode[NMeta] != -1){ // yfu add for MappingMode == -1
...
// yfu add for getting the Nm parameter
int metamorphCollection::GetNm(int ifl)
{
  return MetaRoster[ifl]->GetNm();
}
