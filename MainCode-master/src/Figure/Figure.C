...
void Figure::CustomSetting(int PadNumber, int histNumber1, int histNumber2, TString process)
{
 ...
 if(isFunction[histNumber1]){
   ...
   CustomFunction[PadNumber][Number]->SetFillStyle(0);
   CustomFunction[PadNumber][Number]->SetFillColor(0);
...
void Figure::InputTF1(int Number, TF1* f, const char* legendName)
{
 isInput = true;
...
//     gStyle->SetPalette(104);
     gStyle->SetPalette(56, nullptr, 1.0);
     ...
     if(isCustomGraph[1][1]){
       ...
       while(iPlot <= CustomFigureCount[1]){
         ...
         if(isCustomFunction[1][iPlot]){
           ...
           if(!isResetError[1][iPlot]) CustomFunction[1][iPlot]->Draw("L same");
...
//      if(isHIST) Custom[2][iPlot]->Draw("HIST same");
     ...
     Custom[3][1]->GetYaxis()->SetNdivisions(505);
