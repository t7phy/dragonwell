...
class Graph
{
 ...
 virtual void SetLogX(int xbase = 1){isLogX = true; LogXBase = xbase;};
...
Graph::Graph(const char* FigureName, const char* XTitle, const char* YTitle)
{
 ...
 myColor[6] = (Color_t)kMagenta;
 myColor[7] = (Color_t)kYellow+1;
 ...
 for(int icolor = 27; icolor <= 100; icolor++){
   myColor[icolor] = (Color_t)icolor;
 }
...
void Graph::CustomSetting(int PadNumber, int histNumber1, int histNumber2, TString process)
{
 ...
 for(int ibin = 1; ibin <= CustomHist[PadNumber][Number]->GetNbinsX(); ibin++){
   Custom[PadNumber][Number]->GetXaxis()->SetBinLabel(ibin, CustomHist[PadNumber][Number]->GetXaxis()->GetBinLabel(ibin));
 }

 if(Number == 1) BlankCustomHist[PadNumber] = (TH1D *)CustomHist[PadNumber][Number]->Clone((TString)"BlankHist_" + (int)PadNumber);
 BlankCustomHist[PadNumber]->Reset();
 for(int ibin = 1; ibin <= CustomHist[PadNumber][Number]->GetNbinsX(); ibin++){
   BlankCustomHist[PadNumber]->GetXaxis()->SetBinLabel(ibin, CustomHist[PadNumber][Number]->GetXaxis()->GetBinLabel(ibin));
 }
 ...
 for(int ibin = 1; ibin <= CustomHist[PadNumber][Number]->GetNbinsX(); ibin++){
   ...
   if(isIgnoreError[PadNumber][Number]) Custom[PadNumber][Number]->SetPointError(ibin, BinWidth, 0.0);
   //cout<<CustomHist[PadNumber][Number]->GetBinCenter(ibin)<<"  "<<CustomHist[PadNumber][Number]->GetBinContent(ibin)<<endl;
 }
...
void Graph::SetCustomPlot(int PadNumber, int Number, TString format)
{
 ...
 if(format.Contains("Fill")){
   ...
   Custom[PadNumber][Number]->SetLineWidth(0);
   Custom[PadNumber][Number]->SetFillStyle(1001);
   Custom[PadNumber][Number]->SetFillColor(CustomColor[make_pair(PadNumber, Number)]);
   Custom[PadNumber][Number]->SetFillColorAlpha(CustomColor[make_pair(PadNumber, Number)], 0.2);
   Custom[PadNumber][Number]->SetLineStyle(1);
   Custom[PadNumber][Number]->SetMarkerStyle(45);
   Custom[PadNumber][Number]->SetMarkerStyle(8);
   if(format == "FillL"){
 ...
 //  Custom[PadNumber][Number]->SetMarkerSize(0);
 //  //Custom[PadNumber][Number]->SetLineWidth(0);
 //  Custom[PadNumber][Number]->SetFillStyle(1);
 //  Custom[PadNumber][Number]->SetFillColor(CustomColor[make_pair(PadNumber, Number)]);
 //  Custom[PadNumber][Number]->SetLineStyle(1);
 //  Custom[PadNumber][Number]->SetMarkerStyle(45);
 //  Custom[PadNumber][Number]->SetMarkerStyle(8);
 //  if(format == "FillL") CustomLegendType[PadNumber][Number] = "l";
 //  if(format == "Fill") CustomLegendType[PadNumber][Number] = "f";
 //}
...
void Graph::ScalePlot(int Number, double scale)
{
...
void Graph::Draw()
{
 ...
 if(isCustom){
   ...
   MyN->cd();
   if(isLogX) gPad->SetLogx(LogXBase);
   ...
   if(TotalPadNumber == 1){
     ...
     if(!isSetXRange){
       ...
       //if(!isInputGraph[1]) XRangeMax = h1[1]->GetXaxis()->GetBinUpEdge(Custom[1][1]->GetN());
       //if(!isInputGraph[1]) XRangeMin = h1[1]->GetXaxis()->GetBinLowEdge(1);
       //cout<<"xmin = "<<XRangeMin<<" xmax = "<<XRangeMax<<endl;
     }
     ...
     //for(int ibin = 1; ibin <= Custom[1][1]->GetN(); ibin++){
     //  CustomMultiGraph[1]->GetXaxis()->SetBinLabel(ibin, CustomHist[1][1]->GetXaxis()->GetBinLabel(ibin));
     //}

     if(isLogY){
       Custom[1][1]->SetMinimum(0.0);
       CustomMultiGraph[1]->SetMinimum(0.0);
     }

     int iPlot = 2;
     while(iPlot <= CustomFigureCount[1]){
      if(isLogY) Custom[1][iPlot]->SetMinimum(0.0);
      ...
      if(YRangeMax < Custom[1][iPlot]->GetMaximum()) YRangeMax = Custom[1][iPlot]->GetHistogram()->GetMaximum();
      if(YRangeMin > Custom[1][iPlot]->GetMinimum()) YRangeMin = Custom[1][iPlot]->GetHistogram()->GetMinimum();
     ...
     if(!isInputGraph[1]){
       vector<double> xbins(BlankCustomHist[1]->GetNbinsX() + 1);
       xbins.at(0) = BlankCustomHist[1]->GetXaxis()->GetBinLowEdge(1);
       for(int ibin = 1; ibin <= BlankCustomHist[1]->GetNbinsX(); ibin++){
         xbins.at(ibin) = BlankCustomHist[1]->GetXaxis()->GetBinUpEdge(ibin);
       }
       CustomMultiGraph[1]->GetXaxis()->Set(xbins.size() - 1, &xbins[0]);
       for(int ibin = 1; ibin <= BlankCustomHist[1]->GetNbinsX(); ibin++){
         if((TString)BlankCustomHist[1]->GetXaxis()->GetBinLabel(ibin) != "") CustomMultiGraph[1]->GetXaxis()->SetBinLabel(ibin, BlankCustomHist[1]->GetXaxis()->GetBinLabel(ibin));
       }
     }
