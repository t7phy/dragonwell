...
HMDYSumOfWeight::HMDYSumOfWeight(TString FileList)
{
 ifstream infile;
...
void HMDYSumOfWeight::GetSumOfWeight()
{
 ...
 for(int ifile = 0; ifile < RootNames.size(); ifile++){
   ...
   //cout<<RootNames.at(ifile)<<"  "<<nchannel<<"  "<<Year<<endl;
...
TString HMDYSumOfWeight::GetRootType(TString name)
{
 TString RootType;
...
int HMDYSumOfWeight::GetYear(TString RootName)
{
 int Year = 0;
