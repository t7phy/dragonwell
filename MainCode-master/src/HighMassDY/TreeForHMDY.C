#define TreeForHMDY_cxx
#include "HighMassDY/TreeForHMDY.h"

using namespace std;

void TreeForHMDY::Init()
{
...
void TreeForHMDY::ConfigFile(int NFilePerJob, int iRound)
{
...
void TreeForHMDY::ConfigFile(int ifile)
{
 OneFilePerJob = true;
 FirstFile = 0;
 EndFile = 1;
...
void TreeForHMDY::CloseFile(int ifile)
{
 rootfile[ifile]->Close();
}

void TreeForHMDY::InputFile(vector<TString> RootNames)
{
 this->RootNames = RootNames;
}

void TreeForHMDY::LinkHist(HistsHMDY* myhists)
{
 this->fHist = myhists;
 this->SystematicName = fHist->SystematicName;
}

void TreeForHMDY::SetBranch(int ifile)
{
...
void TreeForHMDY::GetEntry(int ifile, long i)
{
 fChain[ifile]->GetEntry(i);
}

void TreeForHMDY::GenerateFile(TString FileName)
{
 hf = new TFile(FileName, "RECREATE");
}

void TreeForHMDY::DefineTree(int isys, TString TreeName)
{
 ...
 GenerateTree[isys]->Branch("pass_em_TT",                       &pass_em_TT);
 GenerateTree[isys]->Branch("pass_em_TL",                       &pass_em_TL);
 GenerateTree[isys]->Branch("pass_em_LT",                       &pass_em_LT);
 GenerateTree[isys]->Branch("pass_em_LL",                       &pass_em_LL);
 ...
 GenerateTree[isys]->Branch("RecoWeight_El_Stat_up",            &RecoWeight_El_Stat_up,         "RecoWeight_El_Stat_up/D");
 GenerateTree[isys]->Branch("RecoWeight_El_Stat_down",          &RecoWeight_El_Stat_down,       "RecoWeight_El_Stat_down/D");
 GenerateTree[isys]->Branch("RecoWeight_El_PromptSubtr_up",     &RecoWeight_El_PromptSubtr_up,  "RecoWeight_El_PromptSubtr_up/D");
 GenerateTree[isys]->Branch("RecoWeight_El_PromptSubtr_down",   &RecoWeight_El_PromptSubtr_down,"RecoWeight_El_PromptSubtr_down/D");
 GenerateTree[isys]->Branch("RecoWeight_El_RealStat_up",        &RecoWeight_El_RealStat_up,     "RecoWeight_El_RealStat_up/D");
 GenerateTree[isys]->Branch("RecoWeight_El_RealStat_down",      &RecoWeight_El_RealStat_down,   "RecoWeight_El_RealStat_down/D");
 GenerateTree[isys]->Branch("RecoWeight_Mu_Stat_up",            &RecoWeight_Mu_Stat_up,         "RecoWeight_Mu_Stat_up/D");
 GenerateTree[isys]->Branch("RecoWeight_Mu_Stat_down",          &RecoWeight_Mu_Stat_down,       "RecoWeight_Mu_Stat_down/D");
 GenerateTree[isys]->Branch("RecoWeight_Mu_PromptSubtr_up",     &RecoWeight_Mu_PromptSubtr_up,  "RecoWeight_Mu_PromptSubtr_up/D");
 GenerateTree[isys]->Branch("RecoWeight_Mu_PromptSubtr_down",   &RecoWeight_Mu_PromptSubtr_down,"RecoWeight_Mu_PromptSubtr_down/D");
 GenerateTree[isys]->Branch("RecoWeight_Mu_RealStat_up",        &RecoWeight_Mu_RealStat_up,     "RecoWeight_Mu_RealStat_up/D");
 GenerateTree[isys]->Branch("RecoWeight_Mu_RealStat_down",      &RecoWeight_Mu_RealStat_down,   "RecoWeight_Mu_RealStat_down/D");
...
void TreeForHMDY::FillTree(int isys)
{
 GenerateTree[isys]->Fill();
}

void TreeForHMDY::InputSystematic(TString sysName)
{
...
void TreeForHMDY::SystematicSelection(TString sysName, TString AdditionalSelection)
{
...
void TreeForHMDY::BkgSelection(TString name)
{
 if(name == "all") return;
...
TString TreeForHMDY::GetRootType(TString name)
{
 TString RootType;
 ...
 else if(name.Contains("ntuple_MM") || name.Contains("matrix_Data") || name.Contains("Fake_Matrix_Electron") || name.Contains("Fake_Matrix_Muon") || name.Contains("Fake_Matrix_emu")){
   RootType = "Wjets_DD";
 }
...
void TreeForHMDY::updateCutFlow(TH1D *inCutFlow, std::map<std::string, double> *inLabelMap, std::string cutName, double step, bool doUpdate){
  if(!doUpdate) return;
  double Xpos = -1;
