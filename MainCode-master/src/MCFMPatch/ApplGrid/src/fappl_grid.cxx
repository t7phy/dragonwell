...
//yfu
void convolutesubproc_(const int& id, const int& proc, double* data) {
  convolutesubprocwrap_(id, proc, data, fnpdf_, fnalphas_);
}

void convolutesubprocwrap_(const int& id, const int& proc, double* data,
                    void (*pdf)(const double& , const double&, double* ),
                    double (*alphas)(const double& ) ) {
  std::map<int,appl::grid*>::iterator gitr = _grid.find(id);
  if ( gitr!=_grid.end() ) {
    appl::grid*    g = gitr->second;
    std::vector<double> v = g->vconvolute_subproc(proc, pdf, alphas);
    for ( unsigned i=0 ; i<v.size() ; i++ ) data[i] = v[i];
  }
  else throw_exception( "No grid with id ", id );
}
...
//yfu
void convoluteppbarsubproc_(const int& id, const int& proc, double* data) {
  convoluteppbarsubprocwrap_(id, proc, data, fnpdf_, fnalphas_);
}


void convoluteppbarsubprocwrap_(const int& id, const int& proc, double* data,
                         void (*pdf)(const double& , const double&, double* ),
                         double (*alphas)(const double& ) ) {
  std::map<int,appl::grid*>::iterator gitr = _grid.find(id);
  if ( gitr!=_grid.end() ) {
    _pdf = pdf;
    appl::grid* g = gitr->second;
    std::vector<double> v = g->vconvolute_subproc( proc, pdf, antipdf, alphas, g->nloops() );
    for ( unsigned i=0 ; i<v.size() ; i++ ) data[i] = v[i];
    _pdf = 0;
  }
  else throw_exception( "No grid with id ", id );
...
//yfu
void fullconvolutesubprocwrap_(const int& id, const int& proc, double* data,
                        void (*pdf)(const double& , const double&, double* ),
                        double (*alphas)(const double& ),
                        const int& nloops,
                        const double& rscale, const double& fscale  ) {
  ...
  if ( gitr!=_grid.end() ) {
    appl::grid*    g = gitr->second;
    std::vector<double> v = g->vconvolute_subproc( proc, pdf, alphas, nloops, rscale, fscale);
    for ( unsigned i=0 ; i<v.size() ; i++ ) data[i] = v[i];
  }
  else throw_exception( "No grid with id ", id );
}
...
void fullconvolutesubproc_(const int& id, const int& proc, double* data,
                    const int& nloops,
                    const double& rscale, const double& fscale  ) {
  fullconvolutesubprocwrap_( id, proc, data, fnpdf_, fnalphas_, nloops, rscale, fscale);
}
//yfu
