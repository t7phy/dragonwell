module streamlined_interface
  ...
  type(grid_def),     save :: grid, gdarray(6) !yfu
...
subroutine hoppetStartExtended(ymax,dy,Qmin,Qmax,dlnlnQ,nloop,order,factscheme)
  ...
  ! yfu copy from CTEQ-TEA fortran code
  call InitGridDef(gdarray(6),dy/27.0_dp,0.2_dp, order=order)
  call InitGridDef(gdarray(5),dy/9.0_dp, 0.6_dp, order=order) !Lai, replace .5 by .6
  call InitGridDef(gdarray(4),dy/3.0_dp, 2.0_dp, order=order)
  call InitGridDef(gdarray(3),dy/2.0_dp, 5.0_dp, order=order)
  call InitGridDef(gdarray(2),dy/1.5_dp,10.0_dp, order=order)
  call InitGridDef(gdarray(1),dy/1.0_dp,   ymax, order=order) ! ymax=20.8
  call InitGridDef(grid,gdarray(1:6),locked=.true.)
