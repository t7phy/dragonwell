...
module structure_functions
  ...
  use hoppet_cxx_oo_split_mat !yfu
  use qcd
  implicit none
...
!------------------------------------------------------------------------------------------------
! yfu defines C++ wrapper for getting coefficient functions
!------------------------------------------------------------------------------------------------

  function hoppet_get_C2LO() bind(C, name="hoppet_get_C2LO_clone") result(val)
  ...
  function hoppet_get_CLLO() bind(C, name="hoppet_get_CLLO_clone") result(val)
    use iso_c_binding, only: c_double
    implicit none
    real(c_double) :: val
  ...
  function hoppet_get_C3LO() bind(C, name="hoppet_get_C3LO_clone") result(val)
    use iso_c_binding, only: c_double
    implicit none
    real(c_double) :: val
  ...
  function hoppet_get_C2NLO_clone() bind(C, name="hoppet_get_C2NLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%C2NLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%C2NLO))
    end if
  end function

  function hoppet_get_CLNLO_clone() bind(C, name="hoppet_get_CLNLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%CLNLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%CLNLO))
    end if
  end function

  function hoppet_get_C3NLO_clone() bind(C, name="hoppet_get_C3NLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%C3NLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%C3NLO))
    end if
  end function

  function hoppet_get_C2NNLO_clone() bind(C, name="hoppet_get_C2NNLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%C2NNLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%C2NNLO))
    end if
  end function

  function hoppet_get_CLNNLO_clone() bind(C, name="hoppet_get_CLNNLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew
    
    if (.not. associated(ch%CLNNLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%CLNNLO))
    end if
  end function

  function hoppet_get_C3NNLO_clone() bind(C, name="hoppet_get_C3NNLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew
    
    if (.not. associated(ch%C3NNLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%C3NNLO))
    end if
  end function

  function hoppet_get_C2N3LO_clone() bind(C, name="hoppet_get_C2N3LO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew
    
    if (.not. associated(ch%C2N3LO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%C2N3LO))
    end if
  end function

  function hoppet_get_CLN3LO_clone() bind(C, name="hoppet_get_CLN3LO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%CLN3LO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%CLN3LO))
    end if
  end function

  function hoppet_get_C3N3LO_clone() bind(C, name="hoppet_get_C3N3LO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%C3N3LO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%C3N3LO))
    end if
  end function

  function hoppet_get_C2N3LO_fl11_clone() bind(C, name="hoppet_get_C2N3LO_fl11_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%C2N3LO_fl11)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%C2N3LO_fl11))
    end if
  end function

  function hoppet_get_CLN3LO_fl11_clone() bind(C, name="hoppet_get_CLN3LO_fl11_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(ch%CLN3LO_fl11)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(ch%CLN3LO_fl11))
    end if
  end function

  function hoppet_get_P_LO_clone() bind(C, name="hoppet_get_P_LO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(dh%P_LO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(dh%P_LO))
    end if
  end function

  function hoppet_get_P_NLO_clone() bind(C, name="hoppet_get_P_NLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(dh%P_NLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(dh%P_NLO))
    end if
  end function

  function hoppet_get_P_NNLO_clone() bind(C, name="hoppet_get_P_NNLO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(dh%P_NNLO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(dh%P_NNLO))
    end if
  end function

  function hoppet_get_P_N3LO_clone() bind(C, name="hoppet_get_P_N3LO_clone") result(pnew)
    use iso_c_binding
    type(c_ptr) :: pnew

    if (.not. associated(dh%P_N3LO)) then
      pnew = c_null_ptr
    else
      pnew = hoppet_cxx__split_mat__copy(c_loc(dh%P_N3LO))
    end if
  end function

! yfu end define wrapper functions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
