...
C_yfu
      SUBROUTINE MBOOK2(N,TIT,NNBIN,MYBINNING)
      IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
      CHARACTER*(*) TIT
      double precision MYBINNING(NNBIN+1)
      include 'histo.f'
      isIrregBin(N)=1
      NHIST=MAX(N,NHIST)
      TITLE(N)=TIT
      ...
      HINT(N)=0.d0
      do I=1,NBIN(N)+1
        HistBinning(N,I)=MYBINNING(I)
        BINNING(I)=MYBINNING(I)
      enddo
      DO 1 I=1,NBIN(N)
      XHIS(N,I)=(MYBINNING(I)+MYBINNING(I+1))*0.5d0
      IHIS(N,I)=0
   1  HIST(N,I)=0.d0
      END

C_yfu
      SUBROUTINE MFILL2(N,X,Y)
      IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
      include 'histo.f'
      ...
      I=0
      do k=1,NBIN(N)
        if ((X >= HistBinning(N,k)) .and. (X < HistBinning(N,k+1))) then
          I=k
          exit
        endif
      enddo
      ...
      BinWidth=HistBinning(N,I+1)-HistBinning(N,I)
      HIST(N,I)=HIST(N,I)+Y/BinWidth
c     we are renormalising the weights by the bin width
      ELSEIF(I.LE.0) THEN
      IUSCORE(N)=IUSCORE(N)+1
      ELSEIF(I.GT.NBIN(N)) THEN
...
C_yfu
        if ( isIrregBin(I) .eq. 0 ) then
          XSQAVG=XSQAVG/hdel(i)
        else if ( isIrregBin(I) .eq. 1 ) then
          BinWidth=HistBinning(I,L+1)-HistBinning(I,L)
          XSQAVG=XSQAVG/BinWidth
        endif
...
C_yfu
      if (isIrregBin(N) .eq. 0) then
        ...
        BinWidth=HistBinning(N,2)-HistBinning(N,1)
        HINT(N)=XIN*BinWidth
      endif
