...
C_yfu
      subroutine bookplot2(n,tag,titlex,var,wt,wt2,nbin,binning,llplot)
      implicit none
      include 'nplot.f'
      include 'part.f'
      include 'outputflags.f'
      ...
      character*3 llplot
      character*4 tag
      double precision var,wt,wt2,binning(nbin+1)

      if     (tag .eq. 'book') then
        if (dswhisto .eqv. .false.) then
c--- Traditional MCFM histograms
          call mbook2(n,titlex,nbin,binning)
c--- also book the errors now (in maxhisto+n,2*maxhisto+n)
          call mbook2(maxhisto+n,titlex,nbin,binning)
          call mbook2(2*maxhisto+n,titlex,nbin,binning)
          if ( (part .eq. 'real') .or. (part .eq. 'tota')
     &     .or.(part .eq. 'todk')) then
            call mbook2(3*maxhisto+n,titlex,nbin,binning)
        endif
        else
c--- DSW histograms - call hbook booking routine
        endif
...
c--- also book the errors now (in maxhisto+n); fill temp histos for real
          if ((part .eq. 'lord') .or. (part .eq. 'virt')
     &   .or. (part .eq. 'frag')) then
            call mfill2(n,var,wt)
          call mfill2(maxhisto+n,var,wt2)
        else
          call mfill2(3*maxhisto+n,var,wt)
        endif
        else
c--- DSW histograms - call hbook filling routine
c--- note that we divide by # of iterations here since it is only
