...
void ThreadRunPreQBH(Control process, HistsQBH* myhists)
{
     ...
     // Get Fake Factor
     if(process.doFakeRate){
       QBHFakeRate* FakeRate = new QBHFakeRate();
       FakeRate->LinkLog(process.myLog);
       FakeRate->GetFakeFactor(process.RootNames);
       myhists->InputFakeRate(FakeRate);
     }
     ...
     for(int iround = 0; iround < process.TotalRound; iround++){
       ...
       for(int j = 0; j < TotalThread; j++){
         ...
         if(!process.OneFilePerJob){
           if(j == 0){
             //QBHTree[j]->LinkLog(process.myLog);
             //QBHTree[j]->GetFakeFactor();
           }
         }
         else{
           cout<<"ERROR! running type of OneFilePerJob doesn't work for getting fake factors."<<endl;
         }
...
void CondorRunPreQBH(Control process, HistsQBH* myhists)
{
     ...
     QBHTree->LinkLog(process.myLog);
     QBHTree->GetFakeFactor();
