...
cp ${BaseName}.new ${BaseName}.00.inp

sed -i "15c   0    0100111   00      11111" ${BaseName}.00.inp
sed -i "3c '1 Run ID:                   :'   '${BaseName}.00'   '${BaseName}.00'" ${BaseName}.00.inp

if [ -f "run_chi2.sh" ]; then
    rm run_chi2.sh
fi
...
source ~/setup.sh
cd ${WORKDIR}/CentralFit_AlphaS_${alphas[${i}]}
./f06b.x ${BaseName}.00.inp > ${BaseName}.00.con
EOF
chmod +x run_chi2.sh
