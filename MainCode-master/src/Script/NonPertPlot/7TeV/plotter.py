...
def Stringify(dataset):
    ...
    if '8' in name:
        ...
        return name.split('7')[0] + r" $\sqrt{s} = 7$ TeV"
    elif '13' in name:
...
def main():
    ...
    for dataset in DATASETS:
        if "pT" in dataset:
            xlabel = r"$p_T$ [GeV]"
            ylabel = r"$\frac{1}{\sigma}\frac{{\rm d}\sigma}{{\rm d} p_T}$ [pb/GeV]"
        elif "Phi" in dataset:
            xlabel = r"$\phi^{*}_{\eta}$"
            ylabel = r"$\frac{1}{\sigma}\frac{{\rm d}\sigma}{{\rm d} \phi^{*}_{\eta}}$ [pb]"
