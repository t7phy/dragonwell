...
def plot(dataset, scales, pdf, cfg=None, **options):
    ...
    ax.fill_between(edges, scale_up, scale_dw,
                    step='pre', color='tab:blue',
                    alpha=0.5, label=r"Scale $\oplus$ PDF Uncertainty")
    ...
    bx.step(edges, theory_ratio, where='pre', color='tab:blue',
            label="ResBos2 CSS")
    bx.fill_between(edges, pdf_up_ratio, pdf_dw_ratio, step='pre',
                    color='tab:blue',
                    alpha=0.7, label="PDF Uncertainty")
    bx.fill_between(edges, scale_up_ratio,
                    scale_dw_ratio,
                    step='pre', color='tab:blue',
    ...
    # Handle CFG if supplied
    if cfg is not None:
        cfg_wgt, _ = cfg[f"{dataset}_Theory_Final"].to_numpy()
        cfg_wgt = pad_vals(cfg_wgt)
        ax.step(edges, cfg_wgt, where='pre', color='tab:purple',
                label="ResBos2 CFG")
        cfg_ratio = cfg_wgt / data_vals
        bx.step(edges, cfg_ratio, where='pre', color='tab:purple',
                label="ResBos2 CFG")
...
def main():
    ...
    cfg = uproot.open("CFG/MC_results.root")
