...
class ReweightInterface(extended_cmd.Cmd):
    ...
    def compile(self):
        ...
        for onedir in rwgt_dir_possibility:
            ...
            if self.mother:
                ...
                nb_core = 1
            #os.environ['MENUM'] = '3' # yfu new
            #misc.compile(['allmatrix3py.so'], cwd=pdir, nb_core=nb_core) #yfu new
            os.environ['MENUM'] = '2'
    ...
    def load_module(self, metag=1):
        ...
        for onedir in rwgt_dir_possibility:
            ...
            #for tag in [2*metag+1]: # yfu new
                #with misc.TMP_variable(sys, 'path', [pjoin(path_me), pjoin(path_me,'onedir', 'SubProcesses')]+sys.path): #yfu comment out
                with misc.TMP_variable(sys, 'path', [pjoin(path_me), pjoin(path_me, onedir, 'SubProcesses')]+sys.path): # yfu new
            ...
            #all_prefix = [''.join([i.decode(errors='ignore') for i in j]).strip().lower() for j in mymod.get_prefix()] # yfu comment out
            all_prefix = [ # yfu new
                ''.join([
                    (bytes([i]).decode(errors='ignore') if isinstance(i, int) else i.decode(errors='ignore'))
                    for i in j
                ]).strip().lower()
                for j in mymod.get_prefix()
            ] # yfu new
