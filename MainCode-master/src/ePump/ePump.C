...
void ePump::PrepareAMLMScan(int k)
{
...
void ePump::ConstructUpdateMatrix(void)
{
    ...
    if (!DiagonalQuad) {
        ...
        for (int i=0;i<Ni;i++) {//Yao add for initializing z0 and z0sqr
            z0[i]=0.0;
            z0sqr[i]=0.0;
        }
        dz=NewBestFitLinear();
        if(doLMScan) dz = NewBestFitLMScan();//Yao add for LMScan
        ...
        for (int n=0;n<30;n++) {//version 2021 is n<10
        if (n>7) {//version 2021 is n>0
                alpha=1.0;
            }
    ...
    // Recall that TdynBar[i]=1 if not using dynamical tolerances.
        ...
        NewLength = dz;//Yao add for LMScan
...
//        }
    ...
    OldParaError = 0.0;
    NewParaError = 0.0;
    for (int ierr=0;ierr<Ni;ierr++) {
        double Y1 = 0.0, Y2 = 0.0, Y0new = 0.0;
        for (int j = 0; j < Ni; j++){
          double dY1 = 1.0;
          double dY2 = -1.0;
        ...
        Y1 /= sqrt(Pweight + d[ierr]);
        Y2 /= sqrt(Pweight + d[ierr]);
        Y1 += Y0new;
        Y2 += Y0new;

        //cout<<TdynP[ierr]<<"  "<<TdynM[ierr]<<"  "<<TdynBar[ierr]<<"  "<<Y1<<"  "<<Y2<<endl;

        OldParaError += TdynBar[ierr] * TdynBar[ierr];
        NewParaError += TdynBar[ierr] * TdynBar[ierr] * Y1 * Y1;
    }

    OldParaError = sqrt(OldParaError) / sqrt(Ni);
    NewParaError = sqrt(NewParaError) / sqrt(Ni);

    cout<<"ParaError(old/new): "<<OldParaError<<"  "<<NewParaError<<endl;
}
...
double ePump::NewBestFitLMScan(void)
{
...
// Returns length of z0[i] vector
    ...
    for (int k=0;k<Ndata;k++) {
        if (dataIncluded[k]) {
            if(!isLMScanData.at(k)) PrepareAMlinear(k);
            else if(isLMScanData.at(k)) PrepareAMLMScan(k);
        }
    }
    ...
    for (int i=0;i<Ni;i++) {
        ...
        z0sqr[i]+=z0[i]*z0[i];
        dzsqr+=z0sqr[i];
//ePout<<"Yao: z0["<<i<<"] = "<<z0[i]<<endl;
    }
//ePout<<"Yao: dz = "<<sqrt(dzsqr)<<endl;
                 ...
                 // Returns dChi^2 for data set k
{
    if (!dataIncluded[k]) {
       ...
       for (int a=0;a<Na[k];a++) {
           ...
           //ePout<<"yinh test Delta chi2 :  "<<dxa[a]<<"  "<<YY[a]<<"  "<<XE[k][a]<<"  "<<k<<"  "<<a<<endl;
       }
       for (int a=0;a<Na[k];a++) {
...
double ePump::DeltaChiSquareReal(const d_vec &YY, int k)
                 // Returns dChi^2 for data set k, beta is changed for different theory prediction
{
    if (!dataIncluded[k]) {
        return 0.0;
    } else {
        ...
        if(error_type[k] != 1){
        ...
        else{ // yfu2025.9.24 beta should be changed for different theory prediction
            Eigen::VectorXd Data;
            Eigen::VectorXd UnCorr;

            if(EigenData.count(k) > 0){
              Data = EigenData.at(k);
              UnCorr = EigenUnCorr.at(k);
            }
            else{
              Data = Eigen::VectorXd(Na[k]);
              UnCorr = Eigen::VectorXd(Na[k]);
              for(int a = 0; a < Na[k]; a++){
                Data(a) = XE[k][a];
                UnCorr(a) = s[k][a];
              }
              EigenData[k] = Data;
              EigenUnCorr[k] = UnCorr;
            }

            Eigen::VectorXd Theory = Eigen::VectorXd(Na[k]);
            for(int a = 0; a < Na[k]; a++){
              Theory(a) = YY[a];
            }

            Eigen::MatrixXd Beta(Na[k], Nlam[k]);
            Eigen::MatrixXd b(Na[k], Nlam[k]);

            map<int, int> kSysMethod;
            TString DataName = dataset[k];
            if(DataName.Contains("E253")){
              kSysMethod[0] = 0;
              for(int i = 1; i <= 100; i++) kSysMethod[i] = 1;
            }
            else if(DataName.Contains("E544")){
              kSysMethod[0] = 0;
              for(int i = 1; i <= 73; i++) kSysMethod[i] = 1;
            }
            else if(DataName.Contains("E542")){
              kSysMethod[0] = 0;
              for(int i = 1; i <= 24; i++) kSysMethod[i] = 1;
            }
            else if(DataName.Contains("E545")){
              kSysMethod[0] = 0;
              for(int i = 1; i <= 27; i++) kSysMethod[i] = 1;
            }
            else if(DataName.Contains("E553")){
              kSysMethod[0] = 0;
              for(int i = 1; i <= 336; i++) kSysMethod[i] = 1;
            }
            else if(DataName.Contains("E554")){
              kSysMethod[0] = 0;
              for(int i = 1; i <= 342; i++) kSysMethod[i] = 1;
            }
            else if(DataName.Contains("E555")){
              kSysMethod[0] = 0;
              for(int i = 1; i <= 30; i++) kSysMethod[i] = 1;
            }
            else{
              kSysMethod.clear();
            }
            ...
            for(int a = 0; a < Na[k]; a++){
              for(int alpha = 0; alpha < Nlam[k]; alpha++){
                if(kSysMethod.size() == 0){
                  Beta(a, alpha) = CorrErrs[k][a][alpha] * Theory(a);
                }
                else{
                  if(kSysMethod[alpha] == 1) Beta(a, alpha) = CorrErrs[k][a][alpha] * Theory(a);
                  if(kSysMethod[alpha] == 0){
                    Beta(a, alpha) = CorrErrs[k][a][alpha] * Data(a);
                    UnCorr(a) = sqrt(UnCorr(a) * UnCorr(a) + Beta(a, alpha) * Beta(a, alpha));
                    Beta(a, alpha) = 0.0;
                  }
                }
                b(a, alpha) = Beta(a, alpha) / UnCorr(a);
              }
            }

            Eigen::VectorXd d = (Data - Theory);
            Eigen::VectorXd Diff = d;
            for(int ipoint = 0; ipoint < Na[k]; ipoint++){
              d(ipoint) = d(ipoint) / UnCorr(ipoint);
              Diff(ipoint) = Diff(ipoint) / UnCorr(ipoint) / UnCorr(ipoint);
            }

            Eigen::MatrixXd AMatrix = Eigen::MatrixXd::Identity(Nlam[k], Nlam[k]) + b.transpose() * b; // Nlambda by Nlambda
            Eigen::MatrixXd AMatrix_inv = AMatrix.inverse(); // Nlambda by Nlambda
            Eigen::VectorXd BVector = Beta.transpose() * Diff; // Nlambda by Npt * Npt by 1

            Eigen::VectorXd lambda = AMatrix_inv * BVector; // Nlambda by Nlambda * Nlambda by 1
            Eigen::VectorXd r = (Data - Theory - Beta * lambda);
            for(int ipoint = 0; ipoint < Na[k]; ipoint++){
              r(ipoint) = r(ipoint) / UnCorr(ipoint);
            }
            double dc2 = d.dot(d) - BVector.dot(lambda);

            /*if(dataset[k] == "tabsCT18Am248_1164/E542") {
                cout<<"output data file for "<<" Chi2 = "<<dc2<<"  ";
                cout<<endl;
                ofstream output("542.dta");
                output << "Dataset: " << endl;
                output << "Npts = " << Na[k] << ", chi^2 = " << dc2 << " chi^2/N = " << dc2/Na[k] << endl;
                ...
                for(size_t i = 0; i < Na[k]; ++i) {
                    double sign = (Data(i)-Theory(i))/std::abs(Data(i)-Theory(i));
                    double tot_err;

                    tot_err = sqrt(UnCorr(i)*UnCorr(i) + corr_err(i)*corr_err(i));

                    output << uppercase << scientific << setprecision(5) << Data(i) << "  ";
                    output << uppercase << scientific << setprecision(5) << Theory(i) << "  ";
                    output << uppercase << scientific << setprecision(3) << tot_err << "   ";
                    output << fixed << setprecision(3) << Data(i) / Theory(i) << "   ";
                    output << fixed << setprecision(3) << tot_err / Theory(i) << "   ";
                    output << setw(6) << *right << fixed << setprecision(3) << sign*pow((Data(i)-Theory(i))/tot_err, 2) << " ";
                    output << setw(10) << *right << uppercase << scientific << setprecision(3) << corr_err(i) << "  ";
                    output << uppercase << scientific << setprecision(3) << shifted(i) << "  ";
                    output << uppercase << scientific << setprecision(3) << UnCorr(i) << "   ";
                    output << setw(5) << *right << fixed << setprecision(2) << r(i)*r(i)*r(i)/fabs(r(i)) << "       ";
                    output << 1 << endl;
                }
                output.close();
            }*/
...
// yfu add this function
void ePump::PrepareOldChi2()
{
 for (int k=0;k<Ndata;k++) {
...
void ePump::UpdateErrors(const d_vec &YY, d_vec &newYY, double &DY0sym, double &DY0up, double &DY0down,
...
//    double dum;
    ...
    for (int ierr=0;ierr<Ni;ierr++) {
        ...
        TaylorCoeffs(dy,ddy,YY[0],YY[2*ierr+1],YY[2*ierr+2],tp,tm,tbar);
        dY[ierr]=dy;
        ddY[ierr]=ddy;
    }
    ...
    for (int ierr=0;ierr<Ni;ierr++) {
        dY[ierr]=(YY[2*ierr+1]-YY[2*ierr+2])/2.0;
        if (DiagonalQuad) {
            ddY[ierr]=(YY[2*ierr+1]+YY[2*ierr+2]-2.0*YY[0])*z0[ierr];
        } else {
            ddY[ierr]=0.0;
        }
...
/////////////////////////////////////////////////////////////////////////
        ...
        ///////////////////////////////////////////////////
        // yfu add for calculating each new predictions //
        ///////////////////////////////////////////////////

        double Y1 = 0.0, Y2 = 0.0;
        for (int j = 0; j < Ni; j++){
          double dY1 = YY[2 * j + 1] - YY[0];
          double dY2 = YY[0] - YY[2 * j + 2];
        ...
        if(!isfinite(Y1)) Y1 = Y0new;
        if(!isfinite(Y2)) Y2 = Y0new;
...
/////////////////////////////////////////////////////////////////////////
    ...
    for (int ierr=0;ierr<Ni;ierr++) {
        dY[ierr]=MaxComp(YY[2*ierr+1]-YY[0],YY[0]-YY[2*ierr+2]);
    }
    UpdateErrors0(dY,ddY,DY0up,DYnewUp);
    for (int ierr=0;ierr<Ni;ierr++) {
        dY[ierr]=MaxComp(YY[0]-YY[2*ierr+2],YY[2*ierr+1]-YY[0]);
    }
    UpdateErrors0(dY,ddY,DY0down,DYnewDown);
}

//yfu
void ePump::OptimizeErrors(const d_vec &YY, d_vec &newYY)
{
    for (int ierr=0;ierr<Ni;ierr++) {
        double Y1 = 0.0, Y2 = 0.0;
        for (int j = 0; j < Ni; j++){
                 ...
                 // Uses symmetric error vectors.
{
    ...
    d_vec ddY2(Ni);
    for (int ierr=0;ierr<Ni;ierr++) {
        dY1[ierr]=(YY1[2*ierr+1]-YY1[2*ierr+2])/2.0;
        dY2[ierr]=(YY2[2*ierr+1]-YY2[2*ierr+2])/2.0;
        if (DiagonalQuad) {
            ddY1[ierr]=(YY1[2*ierr+1]+YY1[2*ierr+2]-2.0*YY1[0])*z0[ierr];
            ddY2[ierr]=(YY2[2*ierr+1]+YY2[2*ierr+2]-2.0*YY2[0])*z0[ierr];
        } else {
            ddY1[ierr]=0.0;
            ddY2[ierr]=0.0;
        }
...
void ePump::OptimizedEVContributions(void)
{
    ...
    for (int i=0;i<Ndata;i++) {
        for (int a=0;a<Na[i];a++) {
            OptimizeErrors(Xmat[i][a],XmatNew[i][a]);//yfu
            ...
            for (int r=0;r<Ni;r++) {
                ...
                FractionalContribution[i][a][r] = v2; // yfu
                cout<<setw(12)<<fixed<<setprecision(8)<<right;
                ePout<<setw(12)<<fixed<<setprecision(8)<<right;
                cout<<v2;
                ePout<<v2;
...
//yfu
        OldPDF[k].resize(npts+1);
        NewPDF[k].resize(npts+1);

//yfu for getting the AlphaS
        getline(*infile[k],inputstring);
        *outfile[k]<<inputstring<<endl;
        getline(*infile[k],inputstring);
        *outfile[k]<<inputstring<<endl;

        OldPDFAlphaS[k].resize(nt + 1);
        for(int ialpha = 0; ialpha <= nt; ialpha++){
          double skip;
          getline(*infile[k],inputstring);
          *outfile[k]<<inputstring<<endl;
          line.str(inputstring);
          line >> skip >> skip >> OldPDFAlphaS[k].at(ialpha);
          line.clear();
        }
...
// Rotated Eigenset .pds files
        for (int k=0;k<Ni;k++) {
            ...
            *outfile[2*k+2]<<setw(13)<<scientific<<uppercase<<setprecision(5)<<f2;
            NewPDF[2*k+1][i+1] = f1; //yfu
            NewPDF[2*k+2][i+1] = f2; //yfu
            if ((i+1)%6==0) {
                *outfile[2*k+1]<<endl;
                *outfile[2*k+2]<<endl;
            }
...
// Read in headers from .dat file
        ...
        } while (trim(inputstring)!=flavorstring);
    }
...
//    while (inputstring.substr(0,3)!="---") {
    while (1){
        if(inputstring.substr(0,3)=="---"){//yfu add for update subgrid of LHAPDF format
          *outfile[0]<<"---"<<endl;
          for(int k = 1; k < Ni * 2 + 1; k++){
            getline(*infile[k],inputstring);
            *outfile[k]<<"---"<<endl;
          }
          for(int iline = 1; iline <= 3; iline++){
            for(int k = 0; k < Ni * 2 + 1; k++){
              getline(*infile[k],inputstring);
              if(inputstring != "") *outfile[k]<<inputstring<<endl;
            }
          }
...
//    ePout<<" New "<<Chi2newAlt<<endl;
    ...
    for (int k=0;k<Ndata;k++) {
        ...
        for (int a=0;a<Na[k];a++) {
            UpdateErrors(Xmat[k][a],XmatNew[k][a],Dsym,Dup,Ddown,DnewSym,DnewUp,DnewDown,D0new);//yfu add XmatNew
            ...
            if(DoSaveHist){
              ...
              myhists->ObservableData_old[k]->SetBinContent(a + 1, X0[k][a]);
              myhists->ObservableData_new[k]->SetBinContent(a + 1, D0new);
              myhists->PDFError_old[k]->SetBinContent(a + 1, Dsym);
              myhists->PDFError_new[k]->SetBinContent(a + 1, DnewSym);

              if(error_type[k] == 1 || error_type[k] == 2) myhists->ObservableData_old[k]->SetBinError(a + 1, s[k][a]);//Data Uncertainty
              if(error_type[k] == 1 || error_type[k] == 2) myhists->ObservableData_new[k]->SetBinError(a + 1, s[k][a]);
              if(error_type[k] == 3) myhists->ObservableData_old[k]->SetBinError(a + 1, 1.0 / Cm[k][a][a]);
              if(error_type[k] == 3) myhists->ObservableData_new[k]->SetBinError(a + 1, 1.0 / Cm[k][a][a]);
              myhists->Observable_old[k]->SetBinError(a + 1, Dsym);//PDF Uncertainty
              myhists->Observable_new[k]->SetBinError(a + 1, DnewSym);
...
//yfu
        for(int iEV = 0; iEV < 2 * Ni + 1; iEV++){
          vector<double> TheoryTemplate(Na[k]);
          for(int a=0;a<Na[k];a++){
            TheoryTemplate.at(a) = XmatNew[k][a][iEV];
          }
          dChi2newSet[k][iEV] = DeltaChiSquare(TheoryTemplate, k);
          TheoryTemplate.clear();
        }
////////////////////////////////////////////////////////////////////////////////
        cout<<endl;
...
void TaylorCoeffs(double &dx, double &ddx, 
		const double &X0, const double &Xp, const double &Xm, 
		const double &Tp, const double &Tm, const double &Tbar) {
                //Tbar=(Tp+Tm)/2.0/Tfix, Tp and Tm are not dividing Tfix
		//dx=(Xp-Xm)/sqrt(Tp*Tp+Tm*Tm)/sqrt(2);

                dx=((Tm/Tbar)*(Xp-X0)/Tp+(Tp/Tbar)*(X0-Xm)/Tm)/2.0;//Carl
