...
SelectionWZ :: SelectionWZ (const std::string& name,
...
void SelectionWZ :: MuonSelection(xAOD::Muon *mu){
...
void SelectionWZ :: ElectronSelection(xAOD::Electron *el){
...
//    (el)->auxdata<char>("isHard") = ((el)->pt()/GeV > 25.0 && fabs(el_eta) < 2.47 && !(fabs(el_eta)<1.52 && fabs(el_eta)>1.37));
    ...
    if(((el)->auxdata<char>("Loose_VarRadIso"))) (el)->auxdata<char>("baseline") = true;
    if((el)->auxdata<char>("IPID") && ((el)->auxdata<char>("Loose_VarRadIso"))) (el)->auxdata<char>("baselineWithD0Sig") = true;

    (el)->auxdata<char>("MatrixLepton") = true;

}

void SelectionWZ :: FwdElectronSelection(xAOD::Electron *el){
...
void SelectionWZ :: JetSelection(xAOD::Jet *jt){
...
void SelectionWZ :: overlapTag(void){
