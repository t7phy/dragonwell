...
int main(int argc, char** argv)
{
 ...
 vector<TString> PDFNames(nPDF);
 vector<TString> ErrorTypes(nPDF);

 for(int i = 1; i <= nPDF; i++){
   TString PDFName = argv[i * 2];
   PDFName.ReplaceAll("lha_", "");
   PDFNames.at(i - 1) = PDFName;
   ErrorTypes.at(i - 1) = "Symmetry";
   if(PDFNames.at(i - 1).Contains("MSHT") || PDFNames.at(i - 1).Contains("NNPDF") || PDFNames.at(i - 1).Contains("MSTW")) ErrorTypes.at(i - 1) = "Symmetry68";
 }

 run->SetupNesFlavorList(vector<TString>{"u", "d", "ubar", "dbar", "uv", "dv", "dbou", "ubod", "ubou", "dbod", "ubodb", "gluon", "uod", "uvodv", "Rs"});
...
////////////////////////////////////////////////////////////////////////////////////////////
 ...
 run->GenerateErrorAll(PDFNames, ErrorTypes, 3000.0);
